...
- [系统设计 — 推特/微博设计之信息流 newsfeed](#系统设计--推特微博设计之信息流-newsfeed)
  - [任何系统设计的核心思路——从简如繁](#任何系统设计的核心思路从简如繁)
...
1. 明确需求，了解是一个什么样的系统，希望有一些什么功能（5min）
   - 谁会去用这个系统，是ToB还是ToC。
   - 通过对于目标人群的分析，知道系统的复杂压力大不大。
   - 一个工具、系统可能有多个用法，比如github有人提交，有人下拉。需要保存状态和提供服务。
   - 基础的使用功能，可能的分析功能：谁用了，用了多少，统计指标。
2. 进行数据分析（5min）：需要多大的QPS。
3. API 接口定义 （感觉可有可无，但是需要确定返回什么内容，传入什么参数 5min）
...
> 使用关系型数据库的原因
- 关系型数据：典型的就是表的形式，且表中的内容并不是太长。并且每个字段的类型很清晰。
- 事务：有事务需求
- 可以合理的使用索引
...
>数据存储：
- 用户关系 等非常具有规则的数据可以使用mysql存储 （用户信息：关系型数据库）；
- 对于消息内容可以使用消息id + 消息内容的kv方法存储 （内容信息：非关系型数据库）；
- 用户id + 消息id + 时间id + 媒体文件id的方法使用mysql。（meta信息：关系型数据库）
- 用户的媒体文件使用文件数据库S3 （文件数据库）。
...
## 任何系统设计的核心思路——从简如繁
整个系统需要提供的是两个大的内容，提供发微博/收微博的服务 和 微博数据的存储。最简单的架构：用户->服务器(提供发送微博服务)->数据库存储信息<-服务器（刷feed流）<-用户。
...
```SQL
-- 设计的三张表：内容信息表tweets，用户信息表 users，用户关系表meta-follows
SELECT tweets.*, user.* from tweets
join users on tweets.sender_id = users.id  -- 找到用户发送的微博的对应关系
join follows on follows.followee_id = user.id -- 找到自己关注的用户
where follows.follower_id = current_user
```

当然这里也可以**维护每个用户自己的tweet列表**。当用户需要查看自己的timeline时候，就需要提取自己所有关注的人的topK条消息，并进行归并排序了。
...
- 缺陷：写压力很大，所有粉丝都需要被写一条数据即使是僵尸粉。不及时，因为followers可能很多。存在延迟，对于明星的社交网络可能是个问题。
...
- push+pull：普通用户依然push，明星用户采用pull的方法。明星用户维护自己的timeline在chahe里，粉丝需要自己来取。
...
点赞数量，评论数量，转发数量其实不适合存放在表里。应该放在redis里，因为会被经常的更新。可以每天或者每小时把redis里的点赞数量等数据刷到db里。
...
这个Session Table可以存放在cache中也可以DB中。如果用户非常多，可以使用Cache优化。**但是需要警惕全部放在cache中可能导致掉电以后大规模重新登录，数据库压力陡增**。
...
## 防止单点失效（切片，备份）
数据拆分是保证某个库挂了，也不会导致网站百分百不可用。可以采用分库分表。

纵向拆分其实就是按照业务进行拆分，把大表拆开成为小表，我们经常使用的实际上只是表的部分字段。对于**一些热点数据尽量使用小表**。

**防止锁问题**。横向切分是考虑一些巨大的表的拆分，这里的关键在于分表键的选择和分表的方法。**分表键一般需要与业务的应用场景相关**，分表方法包括简单取模等，粗暴的方法存在热点数据、不容易拓展等问题，比较好的方法是采用**一致性哈希**。这样可以最大程度降低在拓展之后需要重新分表的麻烦。但是不利于范围查找。

数据备份就是考虑主从备份，集群。还可以实现**主库写，从库读**，降低压力。

## 一致性哈希算法
将整个hash区间看成一个环，环的大小为 $[0, 2^{64}-1]$。并且引入了虚拟节点的概念(Virtual nodes)的概念。**每个实体机器对应1000个环上的点**。当我们添加一个数据时候，我们计算key的hash值对应到环上一个点。顺时针找到第一个虚拟节点存储。当新加入一台机器进行数据迁移时候，新加入的1000个virtual node都向各自的顺时针的第一个**虚拟节点**要数据。
...
## 备份与复制
**备份一般是周期性的，复制一般是实时的**。备份是容灾的考虑，复制一般是性能上考虑比如读写分离以及集群的热切换。
...
- 需求：QPS+Storage 同样根据DAU可以估测出来QPS，并且考虑峰值的情况，选择合适的存储系统。可以根据URL需要的存储来计算需要的硬盘空间。是否支持自定义短url。
- 服务：只有一个简单的URL转换
- 存储：sql和nosql都可以。
...
> 最简单的设计：user->服务器做一个映射->数据库读取->user。优化：映射算法，存储方案，负载均衡
...
3. **进制转换BASE62**。可以使用的URL包括(0-9, a-z, A-Z)，一共62个，可以变为62进制。相比Base64编码，Base62编码不含有特殊符号+/，主要用于短URL链接地址。每个URL对应一个整数，整数就是数据库表的主键，是自增的。这样6位数可以存放62^6 = 570亿个内容。优点是效率高，**缺点是依赖于自增主键，只能使用关系型数据库**。 
...
其他的全局自增id：redis，mysql，snowflake雪花算法：强依赖时钟。
...
- 场景：
  - 系统需要实时了解到司机的位置，也就是需要由司机位置的上报；是一个**写密集**的需求，因为我们需要**每4s同步一次司机的位置**。假设由200k司机，每秒的写请求就是50k，峰值可以达到150k。对于存储，如果每条location都保存，200k * 86400/4 * 100bytes = 0.5T/day。如果只记录当前的位置，大概也就200k*100bytes=20M；
  - 需要用户能根据地理位置查询附近最近的topK个司机。需要算法去计算当前坐标下给定范围的司机。
- 服务：需要包括两个大的模块，一个是位置的记录，一个是位置的匹配。
- 存储：**显然对于位置的存储时高写的，对于订单的查询是高读的**。并且对于地理信息的存储比较简单，可以用kv键值对即可，这里可以使用高速的redis实现。并进行额外的落盘。对于订单信息的查询因为比较表会比较复杂，这里使用sql数据库比较好。
...
> 最佳存储方法
>
需求：我们**既需要从地理位置得到附近的司机的位置，也需要知道司机的id得到位置**

既然我们是写频繁的操作，我们可以使用redis建立简单的键值对，key是位置，value是司机编号的set。**比如 Driver 的位置如果是 9q9hvt，则存储在 9q9hvt， 9q9hv， 9q9h 这 3 个 key 中就可以，因为四个位置重复的精度是20km了已经，你不会打车20km以上的车辆**。对于6位一致，已经是一公里以内了，也足够精确了。如果匹配成功(距离够近，司机空闲，司机同样接单)，系统返回司机的编号。然后再去数据库或者另外一个redis表中，查询司机的具体经纬度信息即可。
...
- 拓展：解决单点故障问题，通过多台redis可以解决的性能的瓶颈。当然也需要合理的分摊流量，一个比较好的思路是根据城市进行划分。其实就是判断一个点在不在多边形内的一个数学问题。如何判断用户在不在机场区域打车，可以先判断城市，然后再查找用户是否位于特定区域。
...
我们可以设计一个Message table 这个表格是针对**整个app的全部用户的消息的存储**。
|messageId|messageContextUrl|threadId|create time
|-|-|-|-|


额外的，对于每个人都有一个Thread table，

|ThreadId|userFrom| userTo| other|
|-|-|-|-|

两个表通过thread id进行连接，这样设计的好处，对于message table 我们可以很容易的查询到用户的全部发言信息。对于两个人，A-B和B-A是不同的thread。

对于用户进行检索某个对话的信息时候，肯定是会指定Thread id这个参数的，因此我们查询的时候也是可以很容易的查询到某个对话的信息。Thread id这个参数其实表明了一种关系。并且对于用户的对话设置，其实包括了很多私人设置，比如对于聊天的备注和静音设置等。也就是说，**所有用户发送的信息是共用的一张表，而所有用户的对话是另外一张表**。
...
存放64M的文件大概metadata只有64Bytes，这样对于一个非常大的文件，其实是可以把metadata读入内存的。
