- [Spring](#spring)
  - [模块](#模块)
  - [spring和spring boot区别](#spring和spring-boot区别)
- [Spring IOC \& AOP](#spring-ioc--aop)
  - [IoC](#ioc)
    - [IoC的初始化过程](#ioc的初始化过程)
  - [AOP](#aop)
    - [Spring AOP 和 AspectJ AOP 的区别](#spring-aop-和-aspectj-aop-的区别)
    - [AOP具体的原理，实现接口](#aop具体的原理实现接口)
    - [AOP领域概念](#aop领域概念)
- [Spring bean](#spring-bean)
  - [bean 的生命周期](#bean-的生命周期)
  - [bean 的作用域](#bean-的作用域)
  - [单例 bean 的线程安全问题](#单例-bean-的线程安全问题)
  - [bean的循环依赖问题](#bean的循环依赖问题)
  - [beanFactory和ApplicationContext](#beanfactory和applicationcontext)
- [Spring的设计模式](#spring的设计模式)
- [Spring 事务](#spring-事务)
    - [管理事务的方式](#管理事务的方式)
    - [事务隔离级别](#事务隔离级别)
    - [事务传播行为](#事务传播行为)
    - [事务中异常处理 @Transactional(rollbackFor = Exception.class)](#事务中异常处理-transactionalrollbackfor--exceptionclass)
    - [spring事务的实现原理](#spring事务的实现原理)
- [Spring Boot](#spring-boot)
  - [Spring Boot 优点](#spring-boot-优点)
  - [Spring Boot Starters](#spring-boot-starters)
  - [@SpringBootApplication 注解](#springbootapplication-注解)
    - [Spring Boot 的自动配置](#spring-boot-的自动配置)
      - [细节](#细节)
  - [spring 中IoC容器源码](#spring-中ioc容器源码)
  - [Spring Boot 配置](#spring-boot-配置)
    - [读取配置文件的方法](#读取配置文件的方法)
    - [Spring Boot 加载配置文件的优先级](#spring-boot-加载配置文件的优先级)
- [Spring MVC](#spring-mvc)
  - [SpringMVC 工作原理](#springmvc-工作原理)
- [常用的注解有哪些？](#常用的注解有哪些)
  - [@RestController vs @Controller](#restcontroller-vs-controller)
  - [@Autowired存在多个同一类型的bean，如何选择注入](#autowired存在多个同一类型的bean如何选择注入)
...
- [Spring Boot 如何做请求参数校验？](#spring-boot-如何做请求参数校验)
  - [校验注解](#校验注解)
...
- [常用的 Bean 映射工具有哪些？](#常用的-bean-映射工具有哪些)
- [如何写一个自己的starter](#如何写一个自己的starter)
- [Spring Boot 如何监控系统实际运行状况？](#spring-boot-如何监控系统实际运行状况)
...
Spring 是一种轻量级开发框架，提高开发效率以及系统的可维护性。通过 **依赖注入（DI）**实现了**控制反转(IOC)** 和 **面向切面编程(AOP)**。
...
spring虽然是一个轻量的开发，但是需要配置xml等依赖关系。spring boot 通过加入了自动配置，帮我们省了这个麻烦。
# Spring IOC & AOP
## IoC
...
**Spring AOP就是基于动态代理的**，如果要代理的对象，实现了某个接口，那么Spring AOP会使用**JDK Proxy**，去创建代理对象。具体的原理时基于java的反射机制实现的。而对于没有实现接口的对象，就无法使用 JDK Proxy 去进行代理了，这时候Spring AOP会使用**Cglib** ，这时候Spring AOP会使用 **Cglib** 生成一个被代理对象的子类来作为代理，如下图所示：
...
### AOP具体的原理，实现接口
JDK动态代理：当目标对象实现了接口时，Spring AOP会使用JDK动态代理。它**基于Java的反射机制**，创建一个实现相同接口的代理对象。在运行时，**代理对象会拦截目标方法的调用**，并根据配置的切面逻辑（Advice）在目标方法执行前、执行后或抛出异常时执行相应的切面操作（如日志记录、性能监测等）。

CGLIB动态代理：CGLIB（Code Generation Library）是一个开源的高性能字节码生成库，它通过继承目标对象生成代理类。**代理类覆盖了目标对象的所有非final方法，并在方法调用前后织入切面逻辑。**
...
笼统说是四个: 实例化，属性赋值，初始化，销毁 Destruction

![](https://image.51cto.com/files/uploadimg/20110419/0930070.png)

1. BeanFactory读取Bean定义文件，利用 Java Reflection API 创建一个Bean的实例。
2. 依赖注入，利用 `set()`方法设置一些属性值。
3. 如果 Bean 实现了 `BeanNameAware` 接口，调用 `setBeanName()`方法，传入Bean的名字。
4. 如果 Bean 实现了 `BeanClassLoaderAware` 接口，调用 `setBeanClassLoader()`方法，传入 `ClassLoader`对象的实例。(与上面的类似，如果实现了其他 `*.Aware`接口，就调用相应的方法)
...
8. 如果有和加载这个 Bean的 Spring 容器相关的 `BeanPostProcessor` 对象，执行`postProcessAfterInitialization()` 方法

> 生命末尾，当要销毁 Bean 的时候，
9. 如果 Bean 实现了 `DisposableBean` 接口，执行 `destroy()` 方法。
10. 如果 Bean 在配置文件中的定义包含 `destroy-method`` 属性，执行指定的方法。



这里面**BeanPostProcessor**这个类是SpringAOP的关键，可以用于创建代理。 

![在这里插入图片描述](https://img-blog.csdnimg.cn/45b838e5ba554623816406e7b1a3a7a6.png)
...
Spring 通过一个配置文件来描述 Bean 及 Bean 之间的依赖关系，**利用 Java 的反射功能实例化 Bean 并建立 Bean 之间的依赖关系** 。

**BeanFactory 是类的通用工厂，它可以创建并管理各种类的对象**，这些类就是 POJO，Spring 称这些被创建并管理的类对象为 Bean。

ApplicationContext 由 BeanFactory 派生而来，提供了很多实际应用的功能 。 在 BeanFactory 中，很多功能需要以编程的方式实现，而在 **ApplicationContext 中则可以通过配置的方式来实现**。初始化 ApplicationContext 时，根据配置文件的所在路径，来选择 ApplicationContext 的实现类。
...
### 事务中异常处理 @Transactional(rollbackFor = Exception.class)
...
spring boot约定大于配置，以减少开发工作（默认配置可以修改）。
...
这个是因为`@SpringBootApplication`注解的原因。`@SpringBootApplication`，里面有三个核心的注解`SpringBootConfiguration`, `ComponetScan`, `EnableAutoConfiguration`。

- `@EnableAutoConfiguration`：启用 SpringBoot 的自动配置机制。是实现自动装配的重要注解。也是核心。
- `@ComponentScan`： 扫描被`@Component` (`@Service`,`@Controller`)注解的 bean，注解默认会扫描该类所在的包下所有的类。当然也可以自定义某些不扫描的bean。
- `@Configuration`：`springbootConfiguration` 点进去其实就是`Configuration`。允许在上下文中注册额外的 bean 或导入其他配置类
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/b8a5ae30dce74e3e964345735c262a15.png)

## SpringMVC 工作原理

**原理如下图所示：**
![在这里插入图片描述](https://img-blog.csdnimg.cn/badae57c62ed41e68b30be091fc3edd9.png)
