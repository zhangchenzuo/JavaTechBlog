...
>mysql中容易出现的隔离问题包括，
- 修改丢失，
- 脏读（dirty read）、解决方法：通过加锁。
- 不可重复读（nonrepeatable read）、解决方法：一致性非锁定读（MVCC）
- 幻读（phantom read）。通过多版本并发控制（MVCC）+ 间隙锁（Next-Key Locking）
...
因为存在并发，所以是不可能物理恢复的（A线程和B线程同时修改，只能回退A的）。undo存放在数据库的一个特殊段内undo 段，在共享表空间中。除了用户回滚，可以用用于MVCC，可以读取到这一行之前的行版本信息。当然undo 的产生也会伴随redo 的产生。因为其实是逻辑存储了undo 日志。
...
性能优化主要考虑三个方面，查询优化，索引优化，库表结构优化。 最大化利用索引；尽可能避免全表扫描；减少无效数据的查询；
## 如何查看命中索引
...
2. 2. mysql服务器是不是在分析大量超过需要的行。（可以通过explain查看type，速度从慢到快依次是，全表扫描，索引扫描，范围扫描，唯一索引查询，常数引用。）如果发现扫描大量行，但是只是返回少数行。可以看看是不是可以进行覆盖扫描（减少回表）
...
在数据库里面，扫描行数是影响执行代价的因素之一。扫描的行数越少，意味着访问磁盘数据的次数越少，消耗的CPU资源越少。当然，扫描行数并不是唯一的判断标准，优化器还会结合是否使用临时表、是否排序等因素进行综合判断。 比如使用普通索引会考虑回表的代价.

mysql采用了基于成本的优化器，会尝试预测一个查询的执行成本，选择成本最小的一个策略去执行。在数据库里面，扫描行数是影响执行代价的因素之一。扫描的行数越少，意味着访问磁盘数据的次数越少，消耗的CPU资源越少。当然，扫描行数并不是唯一的判断标准，优化器还会结合是否使用临时表、是否排序等因素进行综合判断。需要根据一系列的统计信息：包括表和索引的页面个数，索引的基数（区分度，不同索引值得数量），索引的分布，数据行的长度等。这个优化并不准确，因此可能错过最优策略。
...
twitter的雪花算法，UUID的算法。在不同的分片上也保证id不会重复

2. 分片字段该如何选择

在开始分片之前，我们首先要确定分片字段（也可称为“片键”）。很多常见的例子和场景中是采用ID或者时间字段进行拆分。这也并不绝对的，我的建议是**结合实际业务，通过对系统中执行的sql语句进行统计分析，选择出需要分片的那个表中最频繁被使用，或者最重要的字段来作为分片字段。**

有一些是**按照时间段拆分**，这样的可以保证大小固定，容义范围查找，但是存在问题是热度不一样，服务器负载不均衡。

或者对**数据取模进行拆分**。这样数据均匀，负载均衡。但是缺点是不容易扩容。
